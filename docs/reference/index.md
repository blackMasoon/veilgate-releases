---
title: Reference
description: Detailed reference documentation for configuration, admin API, MCP and CLI.
---

# Reference

This section contains detailed reference material:

- [Configuration reference](./configuration.md) – structure of `veilgate.yaml`.
- [Admin API](./admin-api.md) – OpenAPI-backed reference for the admin server.
- [MCP configuration](./mcp.md) – how to define MCP tools and mappings.
- [CLI commands](./cli.md) – `veilgate` commands and flags.


